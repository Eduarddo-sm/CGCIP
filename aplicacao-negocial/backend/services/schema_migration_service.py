...
MIGRATION_LOCK_ID = 889020260720
...
def run_schema_migrations(database_url: str, root: Path | None = None) -> None:
    if not str(database_url).startswith(("postgresql://", "postgresql+psycopg://", "postgres://")):
        return
    if os.environ.get("NEGOCIAL_SKIP_MIGRATIONS", "").strip().lower() in {"1", "true", "yes"}:
        return
    try:
        import psycopg
        ...
        raise RuntimeError("PostgreSQL requer Alembic e psycopg para validar o schema negocial.") from exc
